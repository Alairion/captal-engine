...
stream::stream(application& application [[maybe_unused]], const physical_device& physical_device, mixer& mixer)
{
    ...
    output_info.suggestedLatency = static_cast<double>(mixer.frame_per_buffer()) / static_cast<double>(mixer.frequency());

    if(auto err{Pa_OpenStream(&m_stream, nullptr, &output_info, static_cast<double>(mixer.frequency()), mixer.frame_per_buffer(), 0, &impl::callback, &mixer)}; err != paNoError)
