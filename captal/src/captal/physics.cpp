...
static inline cpVect tocp(vec2f vec) noexcept
{
...
static inline vec2f fromcp(const cpVect& vec) noexcept
{
...
void physical_collision_arbiter::set_surface_velocity(vec2f surface_velocity) noexcept
{
...
vec2f physical_collision_arbiter::normal() const noexcept
{
    return fromcp(cpArbiterGetNormal(m_arbiter));
}

std::pair<vec2f, vec2f> physical_collision_arbiter::points(std::size_t contact_index) const noexcept
{
...
void physical_world::point_query(vec2f point, float max_distance, group_t group, collision_id_t id, collision_id_t mask, point_query_callback_type callback)
{
...
void physical_world::ray_query(vec2f from, vec2f to, float thickness, group_t group, collision_id_t id, collision_id_t mask, ray_callback_type callback)
{
    const cpShapeFilter filter{cpShapeFilterNew(group, id, mask)};
...
std::vector<physical_world::point_hit> physical_world::point_query(vec2f point, float max_distance, group_t group, collision_id_t id, collision_id_t mask)
{
...
std::vector<physical_world::ray_hit> physical_world::ray_query(vec2f from, vec2f to, float thickness, group_t group, collision_id_t id, collision_id_t mask)
{
...
std::optional<physical_world::point_hit> physical_world::point_query_nearest(vec2f point, float max_distance, group_t group, collision_id_t id, collision_id_t mask)
{
    const cpShapeFilter filter{cpShapeFilterNew(group, id, mask)};
...
std::optional<physical_world::ray_hit> physical_world::ray_query_first(vec2f from, vec2f to, float thickness, group_t group, collision_id_t id, collision_id_t mask)
{
    const cpShapeFilter filter{cpShapeFilterNew(group, id, mask)};
...
void physical_world::set_gravity(vec2f gravity) noexcept
{
...
vec2f physical_world::gravity() const noexcept
{
...
physical_shape::physical_shape(physical_body& body, float radius, vec2f offset)
...
physical_shape::physical_shape(physical_body& body, vec2f first, vec2f second, float thickness)
...
physical_shape::physical_shape(physical_body& body, std::span<const vec2f> points, float radius)
:m_active{true}
{
...
void physical_shape::set_surface_velocity(vec2f surface_velocity) noexcept
{
...
vec2f physical_shape::surface_velocity() const noexcept
{
...
float circle_moment(float mass, float radius, vec2f origin, float inner_radius) noexcept
{
    return fromcp(cpMomentForCircle(tocp(mass), tocp(radius), tocp(inner_radius), tocp(origin)));
}

float segment_moment(float mass, vec2f first, vec2f second, float thickness) noexcept
{
    return fromcp(cpMomentForSegment(tocp(mass), tocp(first), tocp(second), tocp(thickness)));
}

float polygon_moment(float mass, std::span<const vec2f> points, vec2f offset, float radius) noexcept
{
    stack_memory_pool<1024> pool{};
...
void physical_body::apply_force(vec2f force, vec2f point) noexcept
{
    cpBodyApplyForceAtWorldPoint(m_body, tocp(force), tocp(point));
}

void physical_body::apply_local_force(vec2f force, vec2f point) noexcept
{
    cpBodyApplyForceAtLocalPoint(m_body, tocp(force), tocp(point));
}

void physical_body::apply_impulse(vec2f impulse, vec2f point) noexcept
{
    cpBodyApplyImpulseAtWorldPoint(m_body, tocp(impulse), tocp(point));
}

void physical_body::apply_local_impulse(vec2f impulse, vec2f point) noexcept
{
...
void physical_body::set_mass_center(vec2f center) noexcept
{
...
void physical_body::set_position(vec2f position) noexcept
{
...
void physical_body::set_velocity(vec2f velocity) noexcept
{
...
vec2f physical_body::world_to_local(vec2f vec) noexcept
{
    return fromcp(cpBodyWorldToLocal(m_body, tocp(vec)));
}

vec2f physical_body::local_to_world(vec2f vec) noexcept
{
...
vec2f physical_body::mass_center() const noexcept
{
...
vec2f physical_body::position() const noexcept
{
...
vec2f physical_body::velocity() const noexcept
{
...
physical_constraint::physical_constraint(pin_joint_t, physical_body& first, physical_body& second, vec2f first_anchor, vec2f second_anchor)
...
physical_constraint::physical_constraint(slide_joint_t, physical_body& first, physical_body& second, vec2f first_anchor, vec2f second_anchor, float min, float max)
...
physical_constraint::physical_constraint(pivot_joint_t, physical_body& first, physical_body& second, vec2f pivot)
...
physical_constraint::physical_constraint(pivot_joint_t, physical_body& first, physical_body& second, vec2f first_anchor, vec2f second_anchor)
...
physical_constraint::physical_constraint(groove_joint_t, physical_body& first, physical_body& second, vec2f first_groove, vec2f second_groove, vec2f anchor)
...
physical_constraint::physical_constraint(damped_spring_t, physical_body& first, physical_body& second, vec2f first_anchor, vec2f second_anchor, float rest_length, float stiffness, float damping)
...
void physical_constraint::set_pin_joint_first_anchor(vec2f anchor) noexcept
{
    cpPinJointSetAnchorA(m_constraint, tocp(anchor));
}

void physical_constraint::set_pin_joint_second_anchor(vec2f anchor) noexcept
{
...
vec2f physical_constraint::pin_joint_first_anchor() const noexcept
{
    return fromcp(cpPinJointGetAnchorA(m_constraint));
}

vec2f physical_constraint::pin_joint_second_anchor() const noexcept
{
...
void physical_constraint::set_slide_joint_first_anchor(vec2f anchor) noexcept
{
    cpSlideJointSetAnchorA(m_constraint, tocp(anchor));
}

void physical_constraint::set_slide_joint_second_anchor(vec2f anchor) noexcept
{
...
vec2f physical_constraint::slide_joint_first_anchor() const noexcept
{
    return fromcp(cpSlideJointGetAnchorA(m_constraint));
}

vec2f physical_constraint::slide_joint_second_anchor() const noexcept
{
...
void physical_constraint::set_pivot_joint_first_anchor(vec2f anchor) noexcept
{
    cpPivotJointSetAnchorA(m_constraint, tocp(anchor));
}

void physical_constraint::set_pivot_joint_second_anchor(vec2f anchor) noexcept
{
    cpPivotJointSetAnchorB(m_constraint, tocp(anchor));
}

vec2f physical_constraint::pivot_joint_first_anchor() const noexcept
{
    return fromcp(cpPivotJointGetAnchorA(m_constraint));
}

vec2f physical_constraint::pivot_joint_second_anchor() const noexcept
{
    return fromcp(cpPivotJointGetAnchorB(m_constraint));
}


void physical_constraint::set_groove_joint_first_groove(vec2f groove) noexcept
{
    cpGrooveJointSetGrooveA(m_constraint, tocp(groove));
}

void physical_constraint::set_groove_joint_second_groove(vec2f groove) noexcept
{
    cpGrooveJointSetGrooveB(m_constraint, tocp(groove));
}

void physical_constraint::set_groove_joint_anchor(vec2f anchor) noexcept
{
    cpGrooveJointSetAnchorB(m_constraint, tocp(anchor));
}

vec2f physical_constraint::groove_joint_first_groove() const noexcept
{
    return fromcp(cpGrooveJointGetGrooveA(m_constraint));
}

vec2f physical_constraint::groove_joint_second_groove() const noexcept
{
    return fromcp(cpGrooveJointGetGrooveB(m_constraint));
}

vec2f physical_constraint::groove_joint_anchor() const noexcept
{
    return fromcp(cpGrooveJointGetAnchorB(m_constraint));
}


void physical_constraint::set_damped_spring_first_anchor(vec2f anchor) noexcept
{
    cpDampedSpringSetAnchorA(m_constraint, tocp(anchor));
}

void physical_constraint::set_damped_spring_second_anchor(vec2f anchor) noexcept
{
...
vec2f physical_constraint::damped_spring_first_anchor() const noexcept
{
    return fromcp(cpDampedSpringGetAnchorA(m_constraint));
}

vec2f physical_constraint::damped_spring_second_anchor() const noexcept
{
