...
public:
    ...
    texture(std::string_view file, tph::load_from_file_t, tph::sampling_options sampling = tph::sampling_options{});
    texture(std::string_view data, tph::load_from_memory_t, tph::sampling_options sampling = tph::sampling_options{});
    texture(std::uint32_t width, std::uint32_t height, const std::uint8_t* rgba, tph::sampling_options sampling = tph::sampling_options{});
    texture(tph::image image, tph::sampling_options sampling = tph::sampling_options{});
