...
static entt::entity add_player(entt::registry& world, const cpt::physical_world_ptr& physical_world)
{
    ...
    cpt::polygon_ptr sprite{cpt::make_polygon(cpt::ellipse(48.0f, 32.0f), cpt::colors::white)};
    ...
    cpt::physical_body_ptr body{cpt::make_physical_body(physical_world, cpt::physical_body_type::dynamic, 10.0f, cpt::polygon_moment(10.0f, sprite->points()))};
    body->set_position(glm::vec2{320.0f, 240.0f});

     //Let's add some colors !
    for(std::uint32_t i{}; i < std::size(sprite->points()); ++i)
    {
        //It's easier to make a rainbow with HSV
        const float hue{(static_cast<float>(i) / std::size(sprite->points())) * 360.0f};
    ...
    world.assign<cpt::components::drawable>(player, sprite);
    //Once again we reuse cpt::polygon points to create the physical shape.
    world.assign<cpt::components::physical_body>(player, body).add_shape(sprite->points())->set_collision_type(player_type);
    ...
    auto& controller{world.assign<cpt::components::controller>(player, cpt::physical_body_weak_ptr{body})};
...
static entt::entity fill_world(entt::registry& world, const cpt::physical_world_ptr& physical_world)
{
    ...
    for(auto&& position : positions)
    {
        ...
        world.assign<cpt::components::physical_body>(item, std::move(body)).add_shape(24.0f, 24.0f);
    }
